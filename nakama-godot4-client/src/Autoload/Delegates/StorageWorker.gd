...
func get_player_characters_async() -> Array:
	var storage_objects: NakamaAPI.ApiStorageObjects = await _client.read_storage_objects_async(
		_session,
		[NakamaStorageObjectId.new(COLLECTION, KEY_CHARACTERS, _session.user_id)],
	)
	...
	var decoded: Array = json.data.result.characters
	var characters := []
...
func get_last_player_character_async() -> Dictionary:
	var storage_objects: NakamaAPI.ApiStorageObjects =await _client.read_storage_objects_async(
		_session,
		[NakamaStorageObjectId.new(COLLECTION, KEY_LAST_CHARACTER, _session.user_id)],
	)
	...
	var decoded: Dictionary = json.data.result
